...
param AiUnlimitedName string = 'ai-service'
...
@description('base64 content of SSL certificate file for ALB to use, ignored if UseSelfSignedCertForALB is enabled')
@secure()
param ApplicationgateWayCertificate string = ''

@description('password (if present) for certificate, ignored if UseSelfSignedCertForALB is enabled')
@secure()
