...
var workspaceRepository = '${AiUnlimitedName}-workspaces'
var gtwPublicIP = '${AiUnlimitedName}-gtw-inbound'
var gtwHttpListenerCert = '${AiUnlimitedName}-gtw-http-list-cert'
...
module kvIdentity '../modules/identity.bicep' =
  if (UseKeyVault != 'None') {
    scope: rg
...
module vaultSecret '../modules/vault/secret.bicep' =
  if (UseKeyVault != 'None') {
    scope: rg
    name: 'ssl-certificate'
    params: {
      vaultName: KeyVaultName
      name: gtwHttpListenerCert
      value: ApplicationgateWayCertificate
      tags: Tags
    }
    dependsOn: [vaultAccessPolicy]
  }

module vaultAccessPolicy '../modules/vault/access-policy.bicep' =
  if (UseKeyVault != 'None') {
    scope: rg
    name: 'ssl-cert-keyvault-access-policy'
    params: {
      vaultName: KeyVaultName
      accessPolicy: {
        tenantId: subscription().tenantId
        objectId: kvIdentity.outputs.principalId
        permissions: {
          secrets: ['Get', 'Set', 'List']
        }
      }
    }
    dependsOn: [vault, kvIdentity]
  }
...
module alb '../modules/alb.bicep' = {
  ...
  params: {
    ...
    gtwMSI: gtwCertMSI
    gtwSSLCertSecretUri: UseKeyVault != 'None' ? vaultSecret.outputs.secretUri : ''
    gtwSSLCert: UseKeyVault == 'None' ? ApplicationgateWayCertificate : ''
    gtwSSLCertPwd: UseKeyVault == 'None' ? ApplicationgateWayCertificatePassword : ''
    tags: Tags
  }
